...
print(os.getcwd())
from anon.adapted_from_speechbrain.ecapa_tdnn_sb import ECAPA_TDNN
from collections import OrderedDict
from anon.scripts.readwrite import read_raw_mat
import joblib
...
class latentGenerator(Generator):
    ...
    def gen_vpc(self,rttm_info, xv_dir, filename, **kwargs):
        audio_data = kwargs['audio']
        if self.latent and self.xv:
            if self.ssl_type == 'hubert_soft':
                # audio-(batchsize,1,len)
            ...
            with torch.no_grad():
                ...
                for entry in rttm_info:
                    user_id = entry['user']
                    start = entry['start']
                    length = entry['length']
                    audio_part = audio_data[:, int(start*16000):int((start+length)*16000)]
                    if user_id not in spk_audio:
                        spk_audio[user_id] = audio_part
                    else:
                        spk_audio[user_id] = torch.cat([spk_audio[user_id], audio_part], dim=1)
                spk_xv_anon = {}
                for spk in spk_audio:
                    #convervox
                    if 'spk' in spk:
                        xv = read_raw_mat(xv_dir + '/' + filename + '_' + spk + '.xvector', 192)
                        xv = torch.FloatTensor(xv).unsqueeze(0).to(x.device) 
                    elif 'none' in xv_dir:
                        with torch.no_grad():
                            xv_input = self.fbank(spk_audio[spk].squeeze(1))
                            xv_input = self.mean_var_norm(xv_input, torch.ones(xv_input.shape[0]).to(xv_input.device))
                            xv, _ = self.xv_model(xv_input)
                    else:
                        xvname= xv_dir + '/' +  spk + '.xvector'
                        if not os.path.exists(xvname):
                            xvname= xv_dir + '/' +  spk

                        xv = read_raw_mat(xvname, 192)
                        xv = torch.FloatTensor(xv).unsqueeze(0).to(x.device) 
        ...
        for entry in rttm_info:
            ...
            if (round(curpos,2) < round(start,2)):  
                bkg_length = start-curpos
                if bkg_length > 0.1:
                    #16000/160
                    F0_part = kwargs['f0'][:,:, int(curpos*100):int((curpos+bkg_length)*100)]
                    #16000/320
                    content_part = x[:,:, int(curpos*50):int((curpos+bkg_length)*50)]
                    if content_part.shape[-1] * 2  - F0_part.shape[-1] == 1:
                        F0_part = torch.nn.functional.pad(F0_part, (0, 1), 'replicate')

                    if content_part.shape[-1] * 2  - F0_part.shape[-1] == -1:
                        F0_part = F0_part[:,:,:-1]
                    
                    elif content_part.shape[-1] * 2  - F0_part.shape[-1] == -2:
                        content_part = torch.nn.functional.pad(content_part, (0, 1), 'replicate')

                    content_part = self._upsample(content_part, F0_part.shape[-1])
                    content_part = torch.cat([content_part, F0_part], dim=1)
                    #random select spk as bkg spk, it doesn't matter which spk vector you choose
                    spk_vec = random.choice(list(spk_xv_anon.values()))
                    xv_part = self._upsample(spk_vec, content_part.shape[-1])
                    cat_part_all_list.append(torch.cat([content_part, xv_part], dim=1))
            
                
            #16000/160
            if length > 0.1:
                F0_part = kwargs['f0'][:,:, int(start*100):int((start+length)*100)]
                #16000/320
                content_part = x[:,:, int(start*50):int((start+length)*50)]
                if content_part.shape[-1] * 2  - F0_part.shape[-1] == 1:
                    F0_part = torch.nn.functional.pad(F0_part, (0, 1), 'replicate')

                elif content_part.shape[-1] * 2  - F0_part.shape[-1] == -1:
                    F0_part = F0_part[:,:,:-1]
                
                elif content_part.shape[-1] * 2  - F0_part.shape[-1] == -2:
                    content_part = torch.nn.functional.pad(content_part, (0, 1), 'replicate')
                #print(content_part.shape, F0_part.shape)
                content_part = self._upsample(content_part, F0_part.shape[-1])
                content_part = torch.cat([content_part, F0_part], dim=1)
                #random select spk as bkg spk, it doesn't matter which spk vector you choose
                xv_part = self._upsample(spk_xv_anon[user_id], content_part.shape[-1])
                cat_part_all_list.append(torch.cat([content_part, xv_part], dim=1))
                
                curpos = start+length

        #self.ssl_hop_size = 320
        if  round(curpos,2) < ((audio_data.shape[-1]//320) *320)/ 16000:
            bkg_length =  (audio_data.shape[-1]//320) * 0.02 - curpos
            #16000/160
            if bkg_length > 0.1:
                F0_part = kwargs['f0'][:,:, int(curpos*100):int((curpos+bkg_length)*100)]
                #16000/320
        ...
        cat_part_all = torch.cat(cat_part_all_list, -1)


        return super().forward(cat_part_all)
