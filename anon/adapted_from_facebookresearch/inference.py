...
def generate(h, generator, x, rttm_info, filename, xv_dir):
    start = time.time()
    y_g_hat = generator.gen_vpc(rttm_info,xv_dir,filename,**x).to(device)
...
def init_worker(arguments):
    ...
    dataset = latentDataset(file_list, a.input_test_rttm,  -1, h.n_fft, h.num_mels, h.hop_size, h.win_size,
...
def inference(item_index):
    x, gt_audio, _, filename, rttm_info = dataset[item_index]
    ...
    audio, rtf = generate(h, generator, x, rttm_info, filename, str(a.xv_dir))
    ...
    for entry in rttm_info:
        ...
        if (curpos < start):  
            ...
            audio[int(curpos*16000):int((curpos+bkg_length)*16000)] = gt_audio[int(curpos*16000):int((curpos+bkg_length)*16000)].cpu()
        curpos = start+length
    if len(gt_audio) != len(audio):
        print("%s, ft_len:%d, gen_len:%d \n"%(filename, len(gt_audio),len(audio)))
...
def main():
    ...
    parser.add_argument('--input_test_rttm', default=None)
    ...
    parser.add_argument('--xv_dir', default=None)
    ...
    for i in range(0, len(dataset)):
        ...
        bar = progbar(i+1, len(dataset))
        message = f'{bar} {i+1}/{len(dataset)} '
        stream(message)
        inference(i)
