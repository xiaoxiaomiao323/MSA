...
def extract_rttm(rttm):
    speaker = []
    with open(rttm) as f:
        line = f.readline()
        while line:
            #pattern = r"(\d+\.\d+)\s+(\d+\.\d+)\s+<NA>\s+<NA>\s+(\w+)"
    ...
    # newly add start consider overlap
    array_length = len(sorted_data)
    for i in range(array_length - 1):
        if (sorted_data[i]["start"] + sorted_data[i]["length"]) > sorted_data[i+1]["start"]:
            overlap_length = sorted_data[i+1]["start"] - sorted_data[i]["start"]
            if overlap_length <= 0.0:
                print("overlap_length = %f\n"%overlap_length)
                overlap_length = 0.1
            sorted_data[i]["length"] = round(overlap_length,2) 
    sorted_data = sorted(sorted_data, key=lambda x: x['start'])
    # newly add end

    return sorted_data

class latentDataset(torch.utils.data.Dataset):
    def __init__(self, training_files, rttm_dir, segment_size, n_fft, num_mels,
    ...
    def __getitem__(self, index):
        ...
        # 
        rttm = self.rttm_dir + '/' + filename + '.rttm'
        rttm_info = extract_rttm(rttm)
        ...
        return feats,audio.squeeze(0), mel_loss.squeeze(),filename, rttm_info
