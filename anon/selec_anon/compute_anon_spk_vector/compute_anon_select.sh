...
do  
	...
	python anon/selec_anon/compute_anon_spk_vector/concatenate_scp.py ${TMPSCP} ${SUBFILES}
    else
	cp ${XVEC_SCP} ${TMPSCP}
    fi

    echo "python speaker_xvector.py ${SPK2UTT} ${TMPSCP} ${OUTPUTDIR} ${SPKXVECNAME}"
    python anon/selec_anon/compute_anon_spk_vector/speaker_xvector.py ${SPK2UTT} ${TMPSCP} ${OUTPUTDIR} ${SPKXVECNAME}
done
...
do    
    ...
    python anon/selec_anon/compute_anon_spk_vector/compute_spk_pool_cosine.py ${TMP_SRC_PATH} ${TMP_POOL_PATH} ${TMP_OUTPUT_DIR}
done
...
do
    ...
    python anon/selec_anon/compute_anon_spk_vector/gen_pseudo_xvecs.py ${TMP_SPK2UTT} ${TMP_SPK2GENDER} ${TMP_POOL_SPK2GENDER} \
	   ${TMP_AFFINITY_PATH} ${TMP_POOL_DATA_PATH} ${TMP_OUTPUT_DIR} ${PSEUDODIRBINDIR} \
	   ${rand_level} ${cross_gender} ${proximity} ${rand_seed} ${xvec_dim}
