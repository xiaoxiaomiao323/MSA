...
from speaker_encoder_cam import CAMPPlus
...
def compute_embeddings(scp, segments, source, 
	...
	def init_speaker_encoder(source):
		...
		for name, param in loadedState.items():
			if name not in selfState:
				new_name = name.replace('speaker_encoder.', '')				
				if new_name in selfState:
					selfState[new_name].copy_(param)					
			else:
	...
	# with open('/data08/Dihard/third_dihard_challenge_eval/data/num_speaker.json', 'r') as f:
	# 	room_id_dic = json.load(f)
...
def cluster(embeddings, p=.01, min_num_spks=2, max_num_spks=10):
	...
	def spectral(M, min_num_spks, max_num_spks):
	...
	def kmeans(data):
		k = data.shape[1]
		_, labels, _ = k_means(data, k, random_state=323, n_init=10)
		return labels
	...
	spectral_embeddings = spectral(laplacian_matrix, min_num_spks, max_num_spks)
	# Assign class labels
