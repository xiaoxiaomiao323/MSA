...
def read_labels(labels, frame_shift=10):
    ...
    for line in open(labels, 'r'):
        ...
        temp = subseg.split('-')
        utt = "-".join(temp[:-4])
        begin_ms, end_ms, begin_frames, end_frames = temp[-4:]
...
def main():
    ...
    rttm_line_spec = "SPEAKER {} {} {:.3f} {:.3f} <NA> <NA> {}_speaker{} <NA> <NA>"
    for (utt, begin, end, label) in merged_segment_to_labels:
        print(rttm_line_spec.format(utt, channel, begin, end - begin, utt, label))
